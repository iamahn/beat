...
let trainingEnabled = false; // 💡 let 키워드 누락 오류 수정
let barsCompleted = 0;
...
// 🛠️ [긴급 추가] 누락되었던 비주얼라이저 업데이트 기능 복구
...
// =====================================
// =====================================
// AUDIO CORE LOOP (다양한 퍼커션 사운드 엔진 엔진 통합 🥁)
// =====================================
function playClick(accent, isSubdivision = false) {
    const type = soundTypeInput.value;
    const now = audioCtx.currentTime;

    // 예외 처리: Human 음성이 구현되어 있을 경우 서브디비전은 클릭음으로 대체
    ...
    if (currentType === "shaker") {
        ...
        // 강박일 때는 더 뚫고 나오는 소리, 서브디비전일 때는 더 부드러운 소리 처리
        filter.frequency.value = accent ? 6000 : (isSubdivision ? 8500 : 7500); 

        const gain = audioCtx.createGain();
        const volume = accent ? 0.6 : (isSubdivision ? 0.25 : 0.4); // 볼륨 차등 부여
        ...
        noise.start(now);
        return; // 커스텀 오디오 처리가 끝났으므로 하단 공통 로직 실행 방지
    }

    // -----------------------------------------------------------------
    // 2. SNARE (트라이앵글 파형 + 밴드패스 노이즈 하이브리드)
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    // 3. COWBELL (듀얼 스퀘어 파형 불협화 쇳소리 합성)
    // -----------------------------------------------------------------
    if (currentType === "cowbell") {
        // 액센트에 따라 기본 피치를 조절하여 차이 생성
        ...
        osc2.type = "square";
        osc2.frequency.setValueAtTime(baseHz * 1.48, now); // 금속성 배음 비율
    ...
    // -----------------------------------------------------------------
    // 5. 기본 신호음 기반 사운드들 (Click, Wood, Beep) 공통 루프 처리
    // -----------------------------------------------------------------
    const osc = audioCtx.createOscillator();
    const gain = audioCtx.createGain();
    ...
    // 약박이나 서브디비전일 때 소리가 귀에 너무 피로하지 않도록 볼륨 자동 최적화
